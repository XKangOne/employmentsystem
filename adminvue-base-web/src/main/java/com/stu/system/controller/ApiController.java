...
import com.stu.util.MyMD5Util;
import com.stu.util.RedisCache;
import com.stu.vo.*;
...
public class ApiController {
    ...
    public Result findOneusersPage(@RequestParam(required = true, defaultValue = "") String phone) {
        ...
        // 如果用户列表为空（或为null）
        if (users == null) {
            // 抛出自定义的业务异常 BusinessException
            throw new BusinessException(ResultCode.USER_ACCOUNT_NOT_EXIST.getCode(),
                    ResultCode.USER_ACCOUNT_NOT_EXIST.getMessage());
        }
        // 返回一个成功的响应 Result，并将用户列表作为数据返回
    ...
    private QueryWrapper<Resumes> getResumesWrapper(ResumesVO dataVO) {
        ...
        if (dataVO != null) {
            ...
            if (!StringUtils.isEmpty(dataVO.getEducation())) {
                queryWrapper.like("education", dataVO.getEducation().trim());
            }
            queryWrapper.orderByDesc("id");
        }
        return queryWrapper;
...
//        String encryptedPwd = null;
    ...
    public Result saveResumes(@RequestParam(required = true, defaultValue = "") String phone,
        ...
        try {
            ...
            data.setEducation(user.getSeducation());
        ...
        try {
            if (userService.findUsersByTel(user.getStel()).size() == 0 ||
                ...
                for (int i = 0; i < resumes.size(); i++) {
                    ...
                    resumes.get(i).setEducation(user.getSeducation());
    ...
    public Result loginUser(String phone_number, String password, String usertype) {
        try {
            ...
            String token = JwtUtil.createToken(phone_number, password);
            redisCache.set("LOGIN_USER_TOKEN_" + phone_number, token);
        }
    }
    ...
    public Result passApplys(@RequestParam(required = true, defaultValue = "0") Integer id) {
        try {
            ...
            Classes classes = classesService.findClassesByCname(user.getSclass()).get(0);
            int cnums = classes.getCnums();
            classes.setCrates((cnums + 1.0) / classes.getCtotalnums() * 100 + "%");
            classes.setCnums(cnums + 1);
            classesService.updateClasses(classes);

            Colleges colleges = collegesService.findCollegesByCname(user.getScollege()).get(0);
            int cnums2 = colleges.getCnums();
            colleges.setCrates((cnums2 + 1.0) / colleges.getCtotalnums() * 100 + "%");
            colleges.setCnums(cnums2 + 1);
            collegesService.updateColleges(colleges);

            Majors majors = majorsService.findMajorsByCname(user.getSmajor()).get(0);
            int cnums3 = majors.getCnums();
            majors.setCrates((cnums3 + 1.0) / majors.getCtotalnums() * 100 + "%");
            majors.setCnums(cnums3 + 1);
            majorsService.updateMajors(majors);
    ...
    public Result sendMail() {
        try {
            ...
            message.setSubject("高校就业管理系统邮箱验证");
            message.setFrom("1910297447@qq.com");
            message.setTo("1910297447@qq.com");
            message.setSentDate(new Date());
            message.setText("邮箱验证码：123456");
    ...
    public static String url = "E:\\bishe\\graduateEmployment062\\employmentsystemVue\\src\\assets\\uploadImgs\\";
